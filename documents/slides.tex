  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.38\textwidth}
      \begin{tikzpicture}[>=Stealth, thick, node distance=1.5cm]
        ...
        \draw[->, dashed] (cem) -- ++(1.8cm,0) |- (encoder);
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{训练循环中的注意力 CEM}
  \begin{figure}
    ...
    \begin{tikzpicture}[>=Stealth, thick, node distance=2.1cm]
      ...
      \draw[->, dashed] (encoder) -- (cem);
      \draw[->, dashed] (cem) -- ++(0,0.1cm) node[right]{\small $\lambda\nabla L_C$} |- (encoder);
      \draw[->, dashed] (decoder) |- ++(0,-1.2cm) -| (encoder) node[midway, below]{\small $\nabla L_D$};
    \end{tikzpicture}
  \end{figure}
\end{frame}
...
\begin{frame}{下一步计划}
  \begin{itemize}
    ...
    \item 尝试混合策略:Gated pooling 预筛查 $\rightarrow$ Slot 框架精细优化高风险类别。
