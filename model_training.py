...
class SlotAttention(nn.Module):
    ...
    Outputs: [B, S, D]
    """
    def __init__(self, feature_dim, num_slots=8, num_iterations=3, eps: float = 1e-8,
                 mlp_hidden_scale: int = 2, slot_dim: int = None):
        super().__init__()
        self.num_slots = num_slots
        self.num_iterations = num_iterations
        self.feature_dim = feature_dim  # input feature dim
        self.eps = eps
        # slot representation dim (default equals input feature dim)
        self.slot_dim = feature_dim if slot_dim is None else slot_dim

        # Slot initialization parameters (learned Gaussian)
        self.slot_mu = nn.Parameter(torch.empty(1, 1, self.slot_dim))
        self.slot_log_sigma = nn.Parameter(torch.empty(1, 1, self.slot_dim))
        # Glorot/Xavier uniform initialization to match paper's intent
        ...
        # GRU-based slot update (applied per slot)
        self.gru = nn.GRUCell(self.slot_dim, self.slot_dim)

        # MLP after GRU with residual connection
    ...
    def forward(self, inputs: torch.Tensor, return_attn_logits: bool = False):
        # inputs: [B, N, D]
        B, N, D = inputs.shape
        assert D == self.feature_dim

        # Normalize and project inputs once
        ...
        attn_logits_last = None

        for _ in range(self.num_iterations):
            slots_prev = slots
            # Compute attention: competition across slots per token
            slots_norm = self.norm_slots(slots)
            q = self.to_q(slots_norm)  # [B, S, D]
            q = q * self.scale

            # attn_logits: [B, N, S] = k:[B,N,D] · q:[B,S,D]
            attn_logits = torch.einsum('bnd,bsd->bns', k, q)
            attn = F.softmax(attn_logits, dim=-1)  # softmax over slots
            # Weighted-normalized aggregation across tokens (add epsilon then renormalize over tokens)
            attn = attn + self.eps
            attn = attn / (attn.sum(dim=1, keepdim=True))

            # Weighted mean: updates [B, S, D]
            updates = torch.einsum('bns,bnd->bsd', attn, v)

            # GRU-based update per slot
            slots = self.gru(
        ...
        if return_attn_logits:
            return slots, attn_logits_last  # [B,S,slot_dim], [B,N,S]
...
# ----------------------------
# Attention 版的 CEM：按类形成 tokens -> 共享 slots -> 并行 cross-attn
# ----------------------------
class SlotCrossAttentionCEM(nn.Module):
    def __init__(self, feature_dim=128, num_slots=8, num_heads=4, num_iterations=3,
        ...
        self.slots_proj = None  # optional projection if slot_dim != feature_dim

    def forward(self, features, labels, unique_labels):
        """
        features: [B, D]
        labels: [B] (torch.long)
        unique_labels: [C] (torch.unique 的结果；保持 torch)
        return:
          rob_loss: 类内 log-variance 的类平均（条件熵 surrogate）
          intra_mse: 类内 MSE 的类平均（用于监控/辅助）
        """
        ...
        for cls in unique_labels:
            cls_mask = (labels == cls)
            class_feats = features[cls_mask] # [M, D]
            M = class_feats.shape[0]
            if M == 0:
                continue

            # 1) 同类样本 -> tokens -> 共享 slots
            tokens = class_feats.unsqueeze(0) # [1, M, D]
            slots_out = self.slot_attention(tokens) # [1, S, D_slot]
            slots_out = slots_out.squeeze(0) # [S, D_slot]
            # project to feature_dim if needed
            if slots_out.shape[-1] != self.feature_dim:
                if self.slots_proj is None:
                    self.slots_proj = nn.Linear(slots_out.shape[-1], self.feature_dim).to(features.device)
                slots = self.slots_proj(slots_out)
            else:
                slots = slots_out

            # 2) 并行 cross-attn：Q=[M,D], KV=[S,D]（共享）
            enhanced = self.cross_attention(class_feats, slots) # [M, D]
...
class MIA_train: # main class for every thing
    ...
    def compute_class_means(self, features, labels, unique_labels, centroids_list):
        ...
        # rob_loss: 条件熵 surrogate（建议作为 loss 使用）
        # intra_mse: 类内 MSE（监控/可加权合并）
        rob_loss, intra_mse = self.attention_cem(features_flat, labels, unique_labels)

        # 4) 与旧接口对齐：返回 (loss, intra_class_mse)
        loss = rob_loss
        intra_class_mse = intra_mse
    ...
    def train_target_step(self, x_private, label_private, adding_noise,random_ini_centers,centroids_list,client_id=0):
        # 确保每个 step 开始即清空上一步的梯度，避免梯度累积导致更新异常
