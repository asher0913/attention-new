    ...
    def train_target_step(self, x_private, label_private, adding_noise, random_ini_centers, client_id=0):
        ...
        if use_attention:
            ...
            if torch.isnan(feats_flat).any() or torch.isinf(feats_flat).any():
                print("Warning: NaN/Inf detected in features, skipping gated attention CEM")
                rob_loss = torch.zeros((), device=device)
                intra_class_mse = torch.zeros((), device=device)
            else:
                if self.gated_attention_cem is None:
                    hidden_dim = min(512, max(64, feats_flat.size(1) // 4))
                    self.gated_attention_cem = GatedAttentionCEM(
                ...
                if torch.isnan(intra_class_mse) or torch.isinf(intra_class_mse):
                    print("Warning: NaN/Inf in intra_class_mse, resetting to zero")
                    intra_class_mse = torch.zeros((), device=device)
        else:
            rob_loss = torch.zeros((), device=device)
            intra_class_mse = torch.zeros((), device=device)
